...
function WBmi({closeParent}) {
    ProtectiveRoute()
    const thArray = ["USER ID", "REPORT ID" , "PATENT NAME","NIC","REPORT DATE","WEIGHT","BMI","ACTION"];
    ...
    const loadReportData = () => {
        axios.get(`${BASE_URL}/api/v2/reports/wBmiReport?nic=${patientNic}`, AuthConfig())
            .then((res) => {
    ...
    const handleSearch = (event) => {
        axios.get(`${BASE_URL}/api/v2/reports/findAllWBmiBy?nic=${patientNic}`, AuthConfig())
            .then((res) => {
                const tempArray = res.data.map((row) => [
                    ...
                    <div>
                        ...
                        <button type="button" className="btn btn-danger" disabled={isPatient} onClick={()=>handleDeleteWBmi(row.id)}>
    ...
    const handleWbmiSave = (event) =>{
        ...
        axios.post(`${BASE_URL}/api/v2/reports/addWBmi`, formData, AuthConfig())
            .then((res) => {
                alert("WBMI was successfully added")
                setRefresh(false)
    ...
    const handleView = (id) => {
        ...
        axios.get(`${BASE_URL}/api/v2/reports/wBmi?id=${id}`, AuthConfig())
            .then((res) => {
                setBmi(res.data.bmi)
    ...
    const handleDeleteWBmi = (id) =>{
        axios.post(`${BASE_URL}/api/v2/reports/removeWBmi?wbmiId=${id}`,{}, AuthConfig())
            .then((res) => {
                                        ...
                                        <div>
                                            <div className="card" style={{ border: '1px solid rgba(135, 206, 250, 1)' }}>
                                                <div className="card-body">
                                                    <h5 className="card-title">WBMI FORM</h5>
                                                    <hr/>
                                                    <form onSubmit={handleWbmiSave} >

                                                        <div className="row">

                                                            <div className="col-6">
                                                                <div className="form-group">
                                                                    <label htmlFor="fbs">Weight value</label>
                                                                    <input disabled={disabled} type="number" required onChange={handleWeight} value={weight} className="form-control" id="examplefbs"
                                                                           placeholder="enter weight"></input>
                                                                </div>

                                                                <div className="form-group">
                                                                    <label htmlFor="fbs">BMI value</label>
                                                                    <input disabled={disabled} type="number" required onChange={handleBmi} value={bmi} className="form-control" id="examplefbs"
                                                                           placeholder="enter BMI"></input>
                                                                </div>
