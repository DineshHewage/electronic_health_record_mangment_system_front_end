...
function LipidProfile({closeParent}) {
    ProtectiveRoute()
    const thArray = ["USER ID", "REPORT ID" , "PATENT NAME","NIC","REPORT DATE","HDL", "LDL" , "VLDL" , "TAG", "CHDL","ACTION"];
    ...
    const handleVldlChange = (event) => {
        setVldl(event.target.value);
    };
    ...
    const handleChdlChange = (event) => {
        setChdl(event.target.value);
    };
    ...
    const loadReportData = () => {
        axios.get(`${BASE_URL}/api/v2/reports/LipidProfileReport?nic=${patientNic}`, AuthConfig())
            .then((res) => {
    ...
    const handleSearch = (event) => {
        axios.get(`${BASE_URL}/api/v2/reports/findAllLipidProfileBy?nic=${patientNic}`, AuthConfig())
            .then((res) => {
    ...
    const handleBpSave = (event) =>{
        event.preventDefault()
        const formData = new FormData();
        formData.append('hdl', hdl);
        formData.append('ldl', ldl);
        formData.append('vldl', vldl);
        formData.append('tag', tag);
        formData.append('chdl', chdl);
        formData.append('patientId', patientId);
        formData.append('reportedDate', reportedDate);
        formData.append('report', file);

        axios.post(`${BASE_URL}/api/v2/reports/addLipidProfile`, formData, AuthConfig())
            .then((res) => {
                alert("Lipid profile was successfully added")
                setRefresh(false)
    ...
    const handleView = (id) => {
        ...
        axios.get(`${BASE_URL}/api/v2/reports/lipidProfile?id=${id}`, AuthConfig())
            .then((res) => {
                setHdl(res.data.hdl)
                setLdl(res.data.ldl)
                setVldl(res.data.vldl)
                setTag(res.data.tag)
                setChdl(res.data.chdl)
    ...
    const handleDeleteFbs = (id) =>{
        axios.post(`${BASE_URL}/api/v2/reports/removeLipidProfile?lipidProfile=${id}`,{}, AuthConfig())
            .then((res) => {
                                        ...
                                        <div>
                                            <div className="card" style={{ border: '1px solid rgba(135, 206, 250, 1)' }}>
                                                <div className="card-body">
                                                    <h5 className="card-title">Lipid Profile FORM</h5>
                                                    <hr/>
